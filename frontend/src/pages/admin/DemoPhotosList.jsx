  ...
  return (
    <div data-testid="admin-demo-photos-list">
      <div className="flex items-center justify-between mb-8">
        ...
        <Link to="/admin/demo-photos/new" data-testid="new-demo-photo-button" className="flex items-center gap-2 bg-navy text-white px-5 py-2.5 rounded-full hover:bg-gold transition-colors">
          <Plus size={16} /> New Photo
        </Link>
      </div>
      <div className="grid gap-4" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))" }}>
        {photos.map((photo) => (
          <div key={photo.id} data-testid={`admin-demo-photo-${photo.id}`} className="bg-white border border-stone rounded-md overflow-hidden">
            <img src={resolveImageUrl(photo.image.thumb_url)} alt="" className="w-full h-40 object-cover" />
            <div className="p-3">
              <p className="text-xs uppercase text-gold font-semibold mb-1">{photo.room}</p>
              ...
              <div className="flex items-center gap-3">
                ...
                <Link to={`/admin/demo-photos/${photo.id}/edit`} data-testid={`edit-demo-photo-${photo.id}`} className="text-ink/60 hover:text-gold">
                  <Pencil size={16} />
                </Link>
