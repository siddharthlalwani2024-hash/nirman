  ...
  return (
    <div data-testid="admin-tiles-list">
      <div className="flex items-center justify-between mb-8">
        ...
        <Link to="/admin/tiles/new" data-testid="new-tile-button" className="flex items-center gap-2 bg-navy text-white px-5 py-2.5 rounded-full hover:bg-gold transition-colors">
      ...
      <div className="bg-white border border-stone rounded-md divide-y divide-stone">
        {tiles.map((tile) => (
          <div key={tile.id} data-testid={`admin-tile-row-${tile.slug}`} className="flex items-center gap-4 p-4">
            <img src={resolveImageUrl(tile.images?.[0]?.thumb_url)} alt="" className="w-14 h-14 rounded-md object-cover bg-white shrink-0" />
            ...
            <button onClick={() => toggle(tile, "featured")} data-testid={`toggle-featured-${tile.slug}`} title="Toggle featured" className={tile.featured ? "text-gold" : "text-ink/40"}>
            ...
            <Link to={`/admin/tiles/${tile.id}/edit`} data-testid={`edit-tile-${tile.slug}`} className="text-ink/60 hover:text-gold">
              <Pencil size={18} />
            </Link>
