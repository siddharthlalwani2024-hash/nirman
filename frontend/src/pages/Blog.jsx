...
export default function Blog() {
  ...
  return (
    <div data-testid="blog-page" className="max-w-6xl mx-auto px-4 sm:px-6 py-14">
      ...
      <div className="mb-10">
        <p className="text-gold text-sm font-semibold tracking-widest uppercase mb-2">Journal</p>
        <h1 className="font-serif text-3xl sm:text-4xl text-ink">From the Showroom</h1>
      </div>
      <div className="grid gap-8" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))" }}>
        {posts.map((post) => (
          <Link key={post.id} to={`/blog/${post.slug}`} data-testid={`blog-card-${post.slug}`} className="group block">
            <div className="aspect-[16/10] rounded-md overflow-hidden bg-white mb-4">
              {post.cover_image && (
            ...
            <h2 className="font-serif text-xl text-ink group-hover:text-gold transition-colors">{post.title}</h2>
            <p className="text-sm text-ink/60 mt-2 leading-relaxed">{post.excerpt}</p>
          </Link>
        ))}
      </div>
      {posts.length === 0 && <p className="text-ink/60 text-sm">No posts yet.</p>}
    </div>
  );
}
