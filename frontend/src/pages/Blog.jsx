      ...
      <div className="grid gap-8" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))" }}>
        {posts.map((post) => (
          <Link key={post.id} to={`/blog/${post.slug}`} data-testid={`blog-card-${post.slug}`} className="group block">
            <div className="aspect-[16/10] rounded-md overflow-hidden bg-canvasAlt mb-4">
              {post.cover_image && (
            ...
            <h2 className="font-serif text-xl text-ink group-hover:text-clay transition-colors">{post.title}</h2>
