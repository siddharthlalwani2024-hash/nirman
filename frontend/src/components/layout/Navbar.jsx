  ...
  return (
    <header className="sticky top-0 z-40 bg-bone/95 backdrop-blur-md border-b border-greige">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between h-16 lg:h-20">
        <Link to="/" data-testid="navbar-logo" className="font-serif text-xl lg:text-2xl text-charcoal tracking-tight">
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `text-sm font-medium tracking-wide pb-1 border-b-2 transition-colors ${
                  isActive ? "text-clay border-clay" : "text-charcoal border-transparent hover:text-clay hover:border-clay"
                }`
              }
            >
              {l.label}
        ...
        <div className="lg:hidden bg-bone border-t border-greige px-4 pb-4">
          {links.map((l) => (
            <NavLink
              ...
              className={({ isActive }) => `block py-3 text-base border-b border-greige/60 ${isActive ? "text-clay" : "text-charcoal"}`}
            >
              {l.label}
            </NavLink>
          ))}
