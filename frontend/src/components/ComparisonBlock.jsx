...
export function ComparisonBlock() {
  return (
    <section className="bg-canvasAlt py-16 sm:py-24" data-testid="comparison-block">
      <div className="max-w-4xl mx-auto px-4 sm:px-6">
        <Reveal>
          <p className="text-clay text-sm font-bold tracking-widest uppercase mb-2">The difference</p>
          <h2 className="font-serif text-3xl sm:text-4xl text-ink mb-10">Why buy from an authorized dealer</h2>
        </Reveal>
        <Reveal delay={0.08}>
          <div className="bg-canvas rounded-md border border-grout overflow-hidden shadow-soft">
            <div className="grid grid-cols-[1fr_auto_auto] sm:grid-cols-[1fr_140px_140px] text-xs sm:text-sm font-semibold uppercase tracking-wide text-ink/60 border-b border-grout px-4 sm:px-6 py-3">
              <span></span>
            ...
            {ROWS.map((row, i) => (
              <div
                ...
                className={`grid grid-cols-[1fr_auto_auto] sm:grid-cols-[1fr_140px_140px] items-center px-4 sm:px-6 py-4 text-sm text-ink ${
                  i !== ROWS.length - 1 ? "border-b border-grout/60" : ""
                }`}
              >
                <span className="pr-4">{row.label}</span>
                <span className="flex justify-center">
                  {row.us ? <Check size={18} className="text-clay" /> : <X size={18} className="text-ink/30" />}
                </span>
                <span className="flex justify-center">
